...
//	1 done
	...
	// update category_game.xml	if it was found
	if(sysLv2FsStat(cat_path, &stat) ==	0) {
		char cat_path_bak[255];

		// rename category_game.xml	as category_game.xml.bak
		strcat(cat_path, ".bak");
		if(sysLv2FsStat(cat_path_bak, &stat) !=	0)
			sysLv2FsRename(cat_path, cat_path_bak);

		// delete category_game.xml	if still exits
		if(sysLv2FsStat(cat_path, &stat) ==	0)
			sysLv2FsUnlink(cat_path);

		// update category_game.xml
		CopyFile("/dev_hdd0/game/UPDWEBMOD/USRDIR/category_game.xml", cat_path);

		// verify copy
		if(sysLv2FsStat(cat_path_bak, &stat) ==	0)
		{
			if(sysLv2FsStat(cat_path, &stat) !=	0)
				sysLv2FsRename(cat_path_bak, cat_path);	 //restore category_game.xml from category_game.xml.bak
			else
				sysLv2FsUnlink(cat_path_bak); //delete category_game.xml.bak
		}
		else
			CopyFile("/dev_hdd0/game/UPDWEBMOD/USRDIR/category_game.xml", cat_path);  // PANIC!	Try	to copy	again!
	}
