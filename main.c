	...
	if(conn_s_p==0xC0FEBABE || conn_s_p==0xC0FEBAB0)
	{
		...
		//if(cobra_mode)
		{
			...
			else
			{
				...
				{
					cobra_config->spoof_version=0x0465;
					cobra_config->spoof_revision=64266;
				}
			}
...
#ifdef REX_ONLY
						add_check_box("c5" , "showidp", STR_SHOWIDPS,   " : <b>R2+O</b><br>"              , !(webman_config->combo & SHOW_IDPS), buffer);
						add_check_box("c6" , "shutdow", STR_SHUTDOWN2,  " : <b>L3+R2+X</b><br>"           , !(webman_config->combo & SHUT_DOWN), buffer);
						add_check_box("c7" , "restart", STR_RESTART2,   " : <b>L3+R2+O</b></td><td>"      , !(webman_config->combo & RESTARTPS), buffer);
#else
				...
				if (data.len > 0)
				{
					...
					if((data.button[CELL_PAD_BTN_OFFSET_DIGITAL1] & CELL_PAD_CTRL_L3) && (data.button[CELL_PAD_BTN_OFFSET_DIGITAL2] & CELL_PAD_CTRL_L2))
					{
						...
						{ // R3+L3+← REBUG Mode Switcher
							...
							&& (cellFsStat((char*)"/dev_flash/vsh/module/vsh.self.cexsp", &s)==CELL_FS_SUCCEEDED))
							{
...
{
	if(c_firmware==4.65f || c_firmware==4.60f || c_firmware==4.55f || c_firmware==4.53f || c_firmware==4.46f || c_firmware==4.21f || c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.50f)
	{
		if(!initial)
		{
			if(backup[0]==0)
...
patch:
	...
	//if(!cobra_mode)
	{
		...
		if(c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.46f ||
           c_firmware==4.50f || c_firmware==4.53f || c_firmware==4.55f || c_firmware==4.60f || c_firmware==4.65f)
		{	// add and enable lv2 peek/poke + lv1 peek/poke
			...
			if(!dex_mode)
			{
				...
				if(c_firmware==4.60f || c_firmware==4.65f)
				{
	...
	if(!dex_mode)
	{
		...
		if(c_firmware==4.65f)
		{
			...
			pokeq(0x8000000000055C5CULL, 0x386000004E800020ULL ); // fix 0x8001002B error (found by @smhabib)
			lv2poke32(0x8000000000058DB0ULL, 0x60000000);         // fix 0x80010017 error (found by @smhabib)
	...
	if(c_firmware>=4.20f && c_firmware<=4.65f)
	{
	...
	if(c_firmware==4.21f)
		...
		sprintf(expplg, "%s/IEXP0_465.BIN", app_sys);
	else
