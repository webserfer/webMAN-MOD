...
bool language(const char *file_str, char *default_str)
{
    ...
    else
    {
		...
		const char lang_code[24][3]={"EN", "FR", "IT", "ES", "DE", "NL", "PT", "RU", "HU", "PL", "GR", "HR", "BG", "IN", "TR", "AR", "CN", "KR", "JP", "ZH", "DK", "SK", "CZ", "XX"};
...
#else
			if(cellFsStat(MM_ROOT_STD, &buf)==CELL_FS_SUCCEEDED) add_to_map(MM_ROOT_STD, MM_ROOT_STD);
...
void remove_cfw_syscalls()
{
	...
	if(c_firmware==4.66f &&  dex_mode) syscall_table = SYSCALL_TABLE_465D;	else
...
static void get_iso_icon(char *icon, char *param, char *file, int isdir, int ns, int abort_connection)
{
    ...
    {
        if(flen>6 && icon[flen-2]=='.')
			icon[flen-6]=0; // remove file extension (split iso)
        else
...
static void get_default_icon(char *icon, char *param, char *file, int isdir, int ns, int abort_connection)
{
	...
	if(icon[0]!=0 && cellFsStat(icon, &s)==CELL_FS_SUCCEEDED && strlen(icon)>4 && icon[strlen(icon)-4]=='.') return;
...
static void handleclient(u64 conn_s_p)
{
	...
	if(conn_s_p==START_DAEMON || conn_s_p==REFRESH_CONTENT)
	{
		...
		is_rebug=(cellFsStat("/dev_flash/rebug", &buf)==CELL_FS_SUCCEEDED);
...
//
...
#endif
				{
					if(f0==9)//ntfs
						...
						sprintf(param, "%s/%s/%c", drives[f0], paths[f1], d0);
				}
				...
				//led(YELLOW, ON);
				{
						...
						)
					{
						...
						if(is_net)
						{
							...
							{
								if(!has_dirs) has_dirs=((data[v3_entry].name[0]!='.') && (strlen(data[v3_entry].name)==1));
							...
							if(data[v3_entry].is_directory && (f1>1 && f1<7))
...
#endif
						{
							...
							if(is_iso || subfolder || cellFsStat(templn, &buf)==CELL_FS_SUCCEEDED)
							{
								msiz=0;
								if(!is_iso)
								{
								...
								else
								{
...
#endif
									{
										...
										{   // ntfs
											...
											if(f1==3 && strstr(entry.d_name+flen, "ntfs[BDISO]")) templn[strlen(templn)-12]=0;
											if(f1==4 && strstr(entry.d_name+flen, "ntfs[DVDISO]")) templn[strlen(templn)-13]=0;
											if(f1==6 && strstr(entry.d_name+flen, "ntfs[PSXISO]")) templn[strlen(templn)-13]=0;
										}
#endif
										if(templn[strlen(templn)-2]=='.') templn[strlen(templn)-2]=0;
										if(templn[strlen(templn)-4]=='.') templn[strlen(templn)-4]=0;
		...
		//title_foundx:
								...
								sprintf(tempstr, "<Table key=\"%04i\">"
												 ...
												 "</Table>",
									key, icon, templn, param, enc_dir_name, enc_dir_name, (subfolder & 2)?".ISO":".iso", (subfolder & 1)?"":".0", (int)pTick.tick, (f0==9?(char*)"/ntfs/":param), (f0==9?paths[f1]:""));
								else
								sprintf(tempstr, "<Table key=\"%04i\">"
												 "<Pair key=\"icon\"><String>%s</String></Pair>"
												 "<Pair key=\"title\"><String>%s</String></Pair>"
												 ...
												 "<Pair key=\"info\"><String>%s%s</String></Pair>"
												 "</Table>",
									key, icon, templn, param, enc_dir_name, (int)pTick.tick, (f0==9?(char*)"/ntfs/":param), (f0==9?paths[f1]:""));
							...
							else if(!has_dirs) has_dirs=((entry.d_name[0]!='.') && (strlen(entry.d_name)==1));
						}
					}
...
continue_reading_folder_xml:
				if(has_dirs && (f1>1 && f1<7) && d0<'Z' && f0!=9)
				{
	...
	ssend(debug_s, "\r\n");
			...
			if(strstr(param, "%"))
			{
				...
				for(u16 i=0;i<len;i++)
				{
					...
					{
						if(header[i+2]>='0' && header[i+2]<='9') param[pos]=header[i+2]-0x30; else
						if(header[i+2]>='A' && header[i+2]<='F') param[pos]=header[i+2]-0x37; else
						if(header[i+2]>='a' && header[i+2]<='f') param[pos]=header[i+2]-0x57;

						if(header[i+1]>='0' && header[i+1]<='9') param[pos]+=(header[i+1]-0x30)*0x10; else
						if(header[i+1]>='A' && header[i+1]<='F') param[pos]+=(header[i+1]-0x37)*0x10; else
						if(header[i+1]>='a' && header[i+1]<='f') param[pos]+=(header[i+1]-0x57)*0x10;
			...
			//else	// text page
			{
				if(is_binary!=2 && strstr(param, "setup.ps3?"))
				{
					...
					if(strstr(param, "nocov=1")) webman_config->nocov=1;
					...
					*/
					...
					if(strstr(param, "vID1=")) {
						char *pos=strstr(param, "vID1=") + 5;
						for(u8 n=0;n<17;n++) {
					...
					if(strstr(param, "vID2=")) {
						char *pos=strstr(param, "vID2=") + 5;
						for(u8 n=0;n<17;n++) {
							if(pos[n]!='&') {webman_config->vIDPS2[n]=pos[n];webman_config->vIDPS2[n+1]=0;}
							else break;
						}
					}
					if(strstr(param, "vPS1=")) {
						char *pos=strstr(param, "vPS1=") + 5;
						for(u8 n=0;n<17;n++) {
							if(pos[n]!='&') {webman_config->vPSID1[n]=pos[n];webman_config->vPSID1[n+1]=0;}
							else break;
						}
					}
					if(strstr(param, "vPS2=")) {
						char *pos=strstr(param, "vPS2=") + 5;
						for(u8 n=0;n<17;n++) {
					...
					{
						char *pos=strstr(param, "autop=") + 6; u8 n;
						for(n=0;n<255;n++)
						{
							if(pos[n]=='+') webman_config->autoboot_path[n]=' ';    else
							if(pos[n]!='&') webman_config->autoboot_path[n]=pos[n]; else break;
						}
					...
					if(strstr(param, "hurl="))
					{
						char *pos=strstr(param, "hurl=") + 5; u8 n;
						for(n=0;n<255;n++)
						{
							if(pos[n]=='+') webman_config->home_url[n]=' ';    else
							if(pos[n]!='&') webman_config->home_url[n]=pos[n]; else break;
						}
						webman_config->home_url[n]=0;
					}
				}
				...
				if(strstr(param, "cpursx.ps3"))
				{
					...
					if(!webman_config->fanc)
					{
						...
						if(!dex_mode)
						{
							if(c_firmware==4.55f || c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f)
							{
		...
		//sclose(&data_s);
				...
				else
				{
					...
					if(strstr(param, "setup.ps3"))
					{
						...
						add_check_box("nogrp", "1", STR_NOGRP, NULL, (webman_config->nogrp), buffer);
						add_check_box("noset", "1", STR_NOSETUP,  NULL, (webman_config->noset), buffer);
						add_check_box("nocov", "1", STR_MMCOVERS, NULL, (webman_config->nocov), buffer);
...
*/
					...
					if(mount_ps3 || strstr(param, "mount.ps3") || strstr(param, "copy.ps3"))
					{
						...
						else
						{
							...
							int plen=0;
							if(strstr(param, "copy.ps3"))  plen=IS_COPY; else
							...
							if(!(plen==IS_COPY && !copy_in_progress)) mount_with_mm(param+((mount_ps3)?10:plen), 1);
...
//
...
#endif
									{
										if(f0==9)//ntfs
											...
											sprintf(param, "%s/%s/%c", drives[f0], paths[f1], d1);
									}
...
//
	continue_reading_folder_html:
									if(has_dirs && (f1>1 && f1<7) && d1<'Z' && f0!=9)
									{
				...
				if(strcasecmp(cmd, "SITE") == 0)
				{
					if(split == 1)
					{
						...
						if(strcasecmp(cmd, "FLASH") == 0)
						{
							...
							struct CellFsStat s; bool rw_flash=(cellFsStat("/dev_blind", &s)==CELL_FS_SUCCEEDED);

							if(strcasecmp(param, "ON" ) == 0) {if( rw_flash) continue; else rw_flash=true; } else
							if(strcasecmp(param, "OFF") == 0) {if(!rw_flash) continue; else rw_flash=false;} else
							rw_flash=!rw_flash;
...
{
	if(c_firmware==4.65f || c_firmware==4.60f || c_firmware==4.55f || c_firmware==4.53f || c_firmware==4.46f || c_firmware==4.21f || c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.50f || c_firmware==4.66f)
	{
		if(!initial)
		{
			if(backup[0]==0)
...
patch:
	...
	//if(!cobra_mode)
	{
		...
		if(c_firmware==4.30f || c_firmware==4.31f || c_firmware==4.40f || c_firmware==4.41f || c_firmware==4.46f ||
           c_firmware==4.50f || c_firmware==4.53f || c_firmware==4.55f || c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f)
		{	// add and enable lv2 peek/poke + lv1 peek/poke
			...
			if(!dex_mode)
			{
				...
				if(c_firmware==4.60f || c_firmware==4.65f || c_firmware==4.66f)
				{
			...
			else // DEX
			{
				...
				else if(c_firmware==4.65f || c_firmware==4.66f)
				{
	...
	if(!dex_mode)
	{   // CEX
		if(c_firmware==4.21f) {SYSCALL_TABLE = SYSCALL_TABLE_421; base_addr=0x2D0C98; open_hook=0x2C2558;}  else
		if(c_firmware==4.30f) {SYSCALL_TABLE = SYSCALL_TABLE_430; base_addr=0x2D2418; open_hook=0x2C3CD4;}  else
		if(c_firmware==4.31f) {SYSCALL_TABLE = SYSCALL_TABLE_431; base_addr=0x2D2428; open_hook=0x2C3CE0;}  else
		if(c_firmware==4.40f) {SYSCALL_TABLE = SYSCALL_TABLE_440; base_addr=0x2D29A8; open_hook=0x2C4284;}  else
		if(c_firmware==4.41f) {SYSCALL_TABLE = SYSCALL_TABLE_441; base_addr=0x2D29B8; open_hook=0x2C4290;}  else
		if(c_firmware==4.46f) {SYSCALL_TABLE = SYSCALL_TABLE_446; base_addr=0x2D2ED8; open_hook=0x2C47B0;}  else
		if(c_firmware==4.50f) {SYSCALL_TABLE = SYSCALL_TABLE_450; base_addr=0x2D4CB8; open_hook=0x29DD20;}  else
		if(c_firmware==4.53f) {SYSCALL_TABLE = SYSCALL_TABLE_453; base_addr=0x2D4E48; open_hook=0x29DEF8;}  else
	...
	{   // DEX
		if(c_firmware==4.21f) {SYSCALL_TABLE = SYSCALL_TABLE_421D; base_addr=0x2EB418; open_hook=0x2D9718;} else
		if(c_firmware==4.30f) {SYSCALL_TABLE = SYSCALL_TABLE_430D; base_addr=0x2ECB48; open_hook=0x2DAE4C;} else
		if(c_firmware==4.41f) {SYSCALL_TABLE = SYSCALL_TABLE_441D; base_addr=0x2ED418; open_hook=0x2DB73C;} else
		if(c_firmware==4.46f) {SYSCALL_TABLE = SYSCALL_TABLE_446D; base_addr=0x2ED938; open_hook=0x2DBC5C;} else
		if(c_firmware==4.50f) {SYSCALL_TABLE = SYSCALL_TABLE_450D; base_addr=0x2F4778; open_hook=0x2B81E8;} else
	...
	{
		sprintf(path, WMTMP "/last_game.txt"); int fd=0;
		if(cellFsOpen(path, CELL_FS_O_RDONLY, &fd, NULL, 0) == CELL_FS_SUCCEEDED)
		{
			u64 read_e = 0;
			if(cellFsRead(fd, (void *)path, 512, &read_e) == CELL_FS_SUCCEEDED) path[read_e]=0;
		...
		{ /*
			...
			if(usbd<=127)
			{
				...
				if(cellFsStat(aiodata, &buf2)==CELL_FS_SUCCEEDED)
				{
					...
					{
						cellFsLseek(fdw, 0x20+(usbd*sizeof(libfs_new)), CELL_FS_SEEK_SET, &msiz);
						cellFsRead(fdw, (void *)&libfs_new, sizeof(libfs_new), &msiz);
						cellFsClose(fdw);

						if(msiz==sizeof(libfs_new) && cellFsOpen(expplg, CELL_FS_O_CREAT | CELL_FS_O_TRUNC | CELL_FS_O_WRONLY, &fdw, 0,0)==CELL_FS_SUCCE\EDED)
						{
							cellFsWrite(fdw, (void *)&libfs_new, sizeof(libfs_new), &msiz);
							cellFsClose(fdw);
