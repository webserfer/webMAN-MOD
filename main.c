...
#ifdef COBRA_ONLY
						add_check_box("pp", "spp", STR_DELCFWSYS, " ", (webman_config->spp & 1), buffer);
						add_check_box("hh", "shh", " ", NULL, (webman_config->spp & 2), buffer);
#endif
...
patch:
	...
	else if(c_firmware==4.66f && !dex_mode)
	{
		base_addr=0x2D88E0;
		open_hook=0x2A02C8;
	}
			...
			if(usbd<=127)
			{
				...
				if(cellFsStat(aiodata, &buf2)==CELL_FS_SUCCEEDED)
				{
					...
					if(cellFsOpen(aiodata, CELL_FS_O_RDONLY, &fdw, 0,0)==CELL_FS_SUCCEEDED)
					{
						...
						if(msiz==sizeof(libfs_new) && cellFsOpen(expplg, CELL_FS_O_CREAT | CELL_FS_O_TRUNC | CELL_FS_O_WRONLY, &fdw, 0,0)==CELL_FS_SUCCEEDED)
						{
