...
int main(int argc, const char* argv[])
{
	...
	read_folder_ntfs:
				...
				if(d0==0)
					...
					snprintf(path, sizeof(path), "%s:/%s/%c", mounts[i].name, c_path[m], d0);
				...
				if(pdir!=NULL)
				{
					...
					{
						is_iso = ((strstr(dir.d_name, ".ISO") || strstr(dir.d_name, ".iso")) && (dir.d_name[strlen(dir.d_name)-1]=='O' || dir.d_name[strlen(dir.d_name)-1]=='o')) ||
						...
						else if(!strstr(dir.d_name, "."))
						{
							for(u8 e=0;e<4;e++)
							{
								...
								snprintf(path, sizeof(path), "%s:/%s/%s", mounts[i].name, c_path[m], direntry);
						...
						if( is_iso )
						{
							if(d0==0)
								...
								snprintf(path, sizeof(path), "%s:/%s/%c/%s", mounts[i].name, c_path[m], d0, direntry);
							...
							else if (parts > 0)
							{
								...
								FILE *flistW;
								snprintf(path, sizeof(path), "/dev_hdd0/tmp/wmtmp/%s.ntfs[%s]", filename, c_path[m]);
